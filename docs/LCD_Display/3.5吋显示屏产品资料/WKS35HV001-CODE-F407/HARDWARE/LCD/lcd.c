...
//显示边框图片
//LCD分辨率为H*V,背景色为background,边框颜色为color
void display_HDBK(u16 H, u16 V, u16 background, u16 color)
{
	 u16 x,y ;
    for( x = 0 ; x < H ; x++ )	
    	 {
...
//LCD分辨率为H*V
//棋盘格的大小为M*N
void qipan_display(u16 H, u16 V, u8 M, u8 N)
{
  u16 x=0,y=0;
  u8 i,j;
  for(j=0;j<(V/(N+M));j++)   
  {
		for(y=(M+N)*j;y<(M+N)*j+M;y++)
    {
			for(i=0;i<(H/(M+N));i++)
      {
				for(x=(M+N)*i;x<(M+N)*i+M;x++)
        {   
				LCD_SetCursor(x,y);		 
				LCD_WriteRAM_Prepare();	
				LCD->LCD_RAM = 0x0000;   
		    }
				for(x=(M+N)*i+M;x<(M+N)*i+(M+N);x++)
        {      
				LCD_SetCursor(x,y);		 
				LCD_WriteRAM_Prepare();	
				LCD->LCD_RAM = 0xFFFF;   
...
//显示crosstalk画面
//LCD分辨率为H×V，黑方块大小为Ｍ×Ｎ
void display_crosstalk(u16 H, u16 V,u8 M, u8 N)
...
extern unsigned char desktop_320480[];
void display_picture2(void)
{
u16 x,y;
int p=0;
u8 a,b;
...
{ 	
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;//PG15 推挽输出,LCD RESET
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//普通输出模式
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
  GPIO_Init(GPIOG, &GPIO_InitStructure);//初始化 //PG15 推挽输出,LCD RESET
  ...
  lcddev.id = LCD_ReadReg(0x0000);  
	...
	{ 
		...
		LCD_WR_DATA(0x82);
		LCD_WR_REG(0x2c);
	}else if(lcddev.id==0x5510)//NT35510 initial code 
	{
