...
u8 TP_Init(void)
{
  GPIO_InitTypeDef  GPIO_InitStructure;	
	...
	{
		if(GT5668_Init()==0)	//是GT5668
		{ 
			tp_dev.scan=GT5668_Scan;	//扫描函数指向GT5668触摸屏扫描
		}else
		{
			OTT2001A_Init();
	...
	{
		FT5206_Init();
		tp_dev.scan=FT5206_Scan;		//扫描函数指向GT5668触摸屏扫描		
		tp_dev.touchtype|=0X80;			//电容屏 
		tp_dev.touchtype|=lcddev.dir&0X01;//横屏还是竖屏 
		return 0;
	}else if(lcddev.id==0X5310&&GT5668_Init()==0)//3.5'电容触摸屏
	{ 
		tp_dev.scan=GT5668_Scan;
		tp_dev.touchtype|=0X80;	//电容屏 
		tp_dev.touchtype|=lcddev.dir&0X01;//横屏还是竖屏 
		return 0;
	}
